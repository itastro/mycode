...
public class InsuranceServiceImpl implements InsuranceService {
	...
	public PageBean<InsuranceDto> pageQuery1(PageQuery pageQuery, InsSearchParam insSearchParam) {
		...
		if (insSearchParam != null) {
			...
			if (StringUtils.isNotBlank(insSearchParam.getVehicleInspection())) {
				predicate8 = qCarInsurance.vehicleInspection.like("%" + insSearchParam.getVehicleInspection() + "%");

			}
			if (StringUtils.isNotBlank(insSearchParam.getIstatus())) {
				predicate9 = qcar.istatus.like("%" + insSearchParam.getIstatus() + "%");
		...
		JPAQuery<InsuranceDto> query = queryFactory
				...
				.distinct().from(qCarInsurance, qcar).where(qcar.vSn.eq(qCarInsurance.vSn), predicate1, predicate2,
						predicate3, predicate4, predicate5, predicate6, predicate7,predicate8,predicate9,predicate10 )
